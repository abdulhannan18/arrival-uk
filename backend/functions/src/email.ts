...
function buildEmailFailureLogContext(
  errorMessage: string,
  options?: {
    userId?: string;
    ticketId?: string;
...
export const sendWelcomeEmailOnSignup = emailRuntime.auth.user().onCreate(async (user) => {
  ...
  try {
    ...
    functions.logger.error("Failed to send welcome message", buildEmailFailureLogContext(
      ...
      for (const userDoc of usersPage.docs) {
        ...
        pendingSends.push((async () => {
          try {
            ...
            functions.logger.error("Failed to send weekly digest", buildEmailFailureLogContext(
              message,
              { userId: userDoc.id }
            ));
          }
...
export const sendSupportTicketConfirmation = emailRuntime.firestore
  ...
  .onCreate(async (snapshot, context) => {
    ...
    try {
      ...
      functions.logger.error("Failed to send support ticket confirmation", buildEmailFailureLogContext(
        error instanceof Error ? error.message : "unknown_error",
...
export const sendCustomEmail = emailRuntime.https.onCall(async (data, context) => {
  ...
  try {
    ...
    functions.logger.error("Failed to send custom message", buildEmailFailureLogContext(
      error instanceof Error ? error.message : "unknown_error",
      {
        userId: context.auth.uid,
        templateKey,
...
export const unsubscribeWeeklyDigest = emailRuntime.https.onRequest(async (request, response) => {
  ...
  try {
    ...
    functions.logger.error("Failed to unsubscribe weekly digest", buildEmailFailureLogContext(
      error instanceof Error ? error.message : "unknown_error",
      { userId }
    ));
