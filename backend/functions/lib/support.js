...
exports.__private__ = exports.addSupportTicketMessage = exports.createSupportTicket = void 0;
...
function sanitizeSupportMetadata(value) {
    ...
    for (const [rawKey, rawValue] of Object.entries(value)) {
        if (accepted >= MAX_METADATA_ENTRIES)
            break;
...
function validateRequiredTextField(value, fieldName, maxLength) {
...
function validateOptionalTextField(value, maxLength) {
    const normalized = safeString(value);
    if (!normalized)
        return undefined;
    return normalized.slice(0, maxLength);
...
function supportTicketCollection() {
    return db
...
exports.createSupportTicket = functions.https.onCall(async (data, context) => {
    ...
    (0, appCheck_1.assertCallableAppCheck)(context, "createSupportTicket");
    ...
    if (category)
        ticketPayload.category = category;
    if (priority)
        ticketPayload.priority = priority;
...
exports.addSupportTicketMessage = functions.https.onCall(async (data, context) => {
    if (!context.auth) {
        throw new functions.https.HttpsError("unauthenticated", "Authentication required");
    }
    (0, appCheck_1.assertCallableAppCheck)(context, "addSupportTicketMessage");
    const userId = context.auth.uid;
    await (0, rateLimit_1.enforceRateLimit)({
        db,
    ...
    const messageRef = ticketRef.collection(constants_1.Collections.support.messages).doc();
