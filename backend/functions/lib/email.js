...
exports.__private__ = exports.unsubscribeWeeklyDigest = exports.sendCustomEmail = exports.sendSupportTicketConfirmation = exports.sendWeeklyDigestEmail = exports.sendWelcomeEmailOnSignup = void 0;
...
function appName() {
    return (process.env.APP_NAME ||
        functions.config()?.app?.name) ?? "Arrival UK";
}
function appURL() {
    const fallback = "https://arrivaluk.app";
    const configured = (process.env.APP_URL ||
        functions.config()?.app?.url) ?? fallback;
    const sanitized = (0, sanitization_1.sanitizeHTTPSURL)(configured, fallback);
...
function sanitizeURL(raw) {
    return (0, sanitization_1.sanitizeHTTPSURL)(raw, appURL());
}
function unsubscribeSecret() {
    ...
    if (!didWarnMissingUnsubscribeSecret) {
        ...
        functions.logger.warn("Missing unsubscribe secret; weekly digest links will fall back to in-app preferences only");
    }
    return null;
}
...
function buildWeeklyDigestUnsubscribeURL(userId, nowMs = Date.now()) {
...
function welcomeHTML(displayName) {
    const safeDisplayName = (0, sanitization_1.escapeHtml)(displayName);
    const safeAppName = (0, sanitization_1.escapeHtml)(appName());
...
function supportCreatedHTML(displayName, ticketId, subject) {
    const safeDisplayName = (0, sanitization_1.escapeHtml)(displayName);
    const safeTicketId = (0, sanitization_1.escapeHtml)(ticketId);
    const safeSubject = (0, sanitization_1.escapeHtml)(subject);
    return `
  <div style="font-family:-apple-system,Arial,sans-serif;line-height:1.5;color:#111827;">
...
function parseTemplateVariables(value) {
    ...
    for (const [key, current] of Object.entries(raw)) {
        if (acceptedCount >= MAX_TEMPLATE_VARIABLES)
            break;
...
async function writeCustomEmailAudit(userId, templateKey, to, status, errorCode) {
...
function renderCustomTemplate(templateKey, variables) {
    const recipientName = (0, sanitization_1.escapeHtml)(variables.recipientName || "there");
    const ctaURL = sanitizeURL(variables.ctaURL || appURL());
    const message = (0, sanitization_1.escapeHtml)(variables.message || "We have an update for you.");
    const appLabel = (0, sanitization_1.escapeHtml)(appName());
...
exports.sendWelcomeEmailOnSignup = emailRuntime.auth.user().onCreate(async (user) => {
    if (!user.email)
        return;
    try {
        await sendMail({
    ...
    while (true) {
        ...
        for (const userDoc of usersPage.docs) {
            ...
            if (!to || !EMAIL_PATTERN.test(to))
                continue;
            const displayName = (0, sanitization_1.escapeHtml)(user.displayName ?? "there");
            const completedCount = Array.isArray(user.progress?.completedTasks)
                ? user.progress.completedTasks.filter((taskId) => typeof taskId === "string").length
                : 0;
...
exports.sendSupportTicketConfirmation = emailRuntime.firestore
    ...
    try {
        await sendMail({
            ...
            html: supportCreatedHTML(userData?.displayName ?? "there", context.params.ticketId, ticket.subject ?? "Support request"),
        });
    }
    catch (error) {
...
exports.sendCustomEmail = emailRuntime.https.onCall(async (data, context) => {
    ...
    (0, appCheck_1.assertCallableAppCheck)(context, "sendCustomEmail");
    if (!await (0, privileged_1.isPrivilegedCaller)(context, db)) {
    ...
    if (!to || !templateKey) {
        throw new functions.https.HttpsError("invalid-argument", "Fields `to` and `templateKey` are required");
    }
    ...
    if (!EMAIL_PATTERN.test(configuredFromEmail)) {
        throw new functions.https.HttpsError("failed-precondition", "SENDGRID_FROM_EMAIL is missing or invalid");
    }
    // Fail fast for admin-triggered emails if email provider is not configured.
    if (!getSendGridClient()) {
        throw new functions.https.HttpsError("failed-precondition", "SendGrid is not configured on this environment");
    }
...
exports.unsubscribeWeeklyDigest = emailRuntime.https.onRequest(async (request, response) => {
