...
struct ContentView: View {
    ...
    @State private var pendingWidgetRoute: (categoryID: String, taskID: String)?
    ...
    @MainActor
    private func handleContinuedTaskGuideActivity(_ activity: NSUserActivity) {
    ...
    @MainActor
    private func handleContinuedOpenDocumentActivity(_ activity: NSUserActivity) {
    ...
    @MainActor
    private func handleMarketplaceCompletion(_ notification: Notification) {
        guard let userInfo = notification.userInfo else { return }
    ...
    @MainActor
    private func launchMarketplaceService(
        ...
        if let url = result.deepLinkURL, ExternalURLPolicy.isAllowed(url) {
    ...
    @MainActor
    private func handleWidgetRoute(_ url: URL) -> Bool {
        guard url.scheme?.lowercased() == ArrivalWidgetSupport.deepLinkScheme else {
            return false
        }
        ...
        if host == "task" {
            ...
            guard !categoryID.isEmpty, !taskID.isEmpty else {
                return true
            }

            if openTaskFromWidgetRoute(categoryID: categoryID, taskID: taskID) {
                return true
            }

            pendingWidgetRoute = (categoryID: categoryID, taskID: taskID)
            return true
        }

        if host == "wallet" {
            let unlockValue = components.queryItems?.first(where: { $0.name == "unlock" })?.value?
                .trimmingCharacters(in: .whitespacesAndNewlines)
                .lowercased()
            let shouldUnlock = unlockValue.map { ["1", "true", "yes", "y"].contains($0) } ?? true
            let documentValue = components.queryItems?.first(where: { $0.name == "document" })?.value
            let documentType = secureDocTypeFromRoute(documentValue)
            openWalletFromWidgetRoute(shouldUnlock: shouldUnlock, preferredDocumentType: documentType)
            return true
        }

        if host == ArrivalWidgetSupport.quickTaskHost {
            return openQuickTaskFromRoute()
        }

        if host == ArrivalWidgetSupport.discountQRHost {
            closeCategoryDetail()
            dismissActiveModal()
            isProfileSheetPresented = false
            presentSheet(.scanQR)
    ...
    @MainActor
    private func consumePendingWidgetRouteIfPossible() {
    ...
    @MainActor
    private func openTaskFromWidgetRoute(categoryID: String, taskID: String) -> Bool {
        guard let categoryIndex = store.categories.firstIndex(where: { $0.id == categoryID }) else {
            return false
        }
    ...
    @MainActor
    private func openWalletFromWidgetRoute(
        shouldUnlock: Bool,
    ...
    @MainActor
    private func openQuickTaskFromRoute() -> Bool {
    ...
    private func secureDocTypeFromRoute(_ rawValue: String?) -> SecureDocType? {
